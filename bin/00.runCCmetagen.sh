...
echo "Deleting directory for job" $KMA_dir/${subdir_name}
...
do
    ...
    if [[ -f "${scratch_dir}/${subdir_name}/${accession}_1.fastq" && -f "${scratch_dir}/${subdir_name}/${accession}_2.fastq" ]]
    then
		...
		find "${KMA_dir}/${subdir_name}/${accession}.fastq.res" -type f -empty -print -delete
		CCMetagen.py -i "${KMA_dir}/${subdir_name}/${accession}.fastq.res" -o ${CC_dir}/${subdir_name}/${subdir_name}
		echo "Fastq, kma and CCMetagen steps completed"
		ls ${CC_dir}/${subdir_name} -ltrh
	elif [[ -f "${scratch_dir}/${subdir_name}/${accession}.fastq" ]]
    then
        # File is single-end
        echo "Processing single-end file for $accession"
        kma -i "${scratch_dir}/${subdir_name}/${accession}.fastq" -o $o -t_db $nt_db -t $threads -1t1 -mem_mode -and
		#Some files have 0MB. Remove empty .res files from analysis.
		find "${KMA_dir}/${subdir_name}/${accession}.fastq.res" -type f -empty -print -delete
		CCMetagen.py -i "${KMA_dir}/${subdir_name}/${accession}.fastq.res" -o ${CC_dir}/${subdir_name}/${subdir_name}
